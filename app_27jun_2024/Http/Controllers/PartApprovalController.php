...
class PartApprovalController extends Controller
{
    /**
    ...
    {
        Session::put("page-title","Part Pending Approval");
...
WHERE 1=1";
        ...
        if(empty($whereTag))
        {
            $tag_data_qr = "select * from tagging_master where case_close is null and call_status='Part Pending' and date(created_at)=curdate();"; 
        }
        else
        {
            $tag_data_qr = "select * from tagging_master where case_close is null and call_status='Part Pending' $whereTag"; 
        }
    ...
    public function view_tag_case(Request $request)
    {
        ...
        $data_json = TaggingMaster::whereRaw("TagId='$TagId' and case_close is null")->first();
        ...
        //print_r($data); exit;
        return view('part-tag-case-view')
                ->with('data',$data)
                ->with('TagId',$TagId)
                ->with('whereTag',$whereTag)
    ...
    {
        //$rma_approve = $_POST['part_approve'];
        $case_arr = $_POST['case'];
        $docket_no = $_POST['docket_no'];
        $courier_det = addslashes($_POST['courier_det']);
        $part_name = $_POST['part_name'];
        $part_remark = $_POST['part_remark'];
        ...
        $UserId = Session::get('UserId');
            ...
            if(1)
            {
                TaggingMaster::whereRaw("TagId='$case_id' and case_close is null")
                        ->update(array('docket_no'=>$docket_no,'courier_det'=>$courier_det,
                            'part_name'=>$part_name,'part_remark'=>$part_remark,'part_approve'=>'Yes','part_status'=>'2',
                            'part_date'=>$cur_date,'part_by'=>$UserId,'case_close'=>'1'));
            }
//            else
//            {
//                TaggingMaster::whereRaw("rma_status='1' and case_close is null")
//                        ->update(array('rma_remark'=>$rma_remark,'rma_approve'=>$rma_approve,'rma_status'=>'1','rma_date'=>$cur_date));
//            }
          
        Session::flash('message', "Dispatch Details Saved Successfully.");
            Session::flash('alert-class', 'alert-success');
        return redirect("part-tag-view?".$whereTag); 
    }
    
    public function save_multi_rma(Request $request)
    {
        exit;
        $part_approve = $_POST['part_approve'];
        $case_arr = $_POST['case'];
        $whereTag = base64_decode($request->input('whereTag')); 
        ...
        foreach($case_arr as $case_id)    
        {
            ...
            {
                TaggingMaster::whereRaw("rma_status='1' and case_close is null")
                        ->update(array('rma_approve'=>$rma_approve,'rma_status'=>'2','rma_date'=>$cur_date,'case_close'=>'1'));
            }
            else
            {
                TaggingMaster::whereRaw("rma_status='1' and case_close is null")
                        ->update(array('rma_approve'=>$rma_approve,'rma_status'=>'1','rma_date'=>$cur_date));
            }
