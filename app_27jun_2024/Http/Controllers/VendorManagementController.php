...
class VendorManagementController extends Controller
{
    ...
    public function get_part_po_no($sr_no,$po_type)
    {
        ...
        {
            $sr_no = 1;
            $part_po_no = $po_tag."$fin_year/".'0001';
            $no = '1'; 
        }
        else
        {
            ...
            $part_po_no = $po_tag."$fin_year/".$new_no;
        }
    ...
    public function view_complaint(Request $request)
    {
        ...
        if(empty($whereTag))
        {
           ...
           $tag_data_qr = "select tm.*,dm.dist_name,tsc.center_name from $from_table tagging_master tm $on_table left join tbl_service_centre tsc on tm.center_id = tsc.center_id left join district_master dm on tm.dist_id = dm.dist_id where job_accept='1' and case_close is null and special_approval_status is null $whereUser  ";
        }
        else
        {
            ...
            //$tag_data_qr = "select tm.*,dm.dist_name,tsc.center_name from $from_table tagging_master tm $on_table left join tbl_service_centre tsc on tm.center_id = tsc.center_id left join district_master dm on tm.dist_id = dm.dist_id where job_reject='0'  $whereUser    $whereTag";   
            $tag_data_qr = "select tm.*,dm.dist_name,tsc.center_name from $from_table tagging_master tm $on_table left join tbl_service_centre tsc on tm.center_id = tsc.center_id left join district_master dm on tm.dist_id = dm.dist_id where job_reject='0' and job_accept='1' and case_close is null and special_approval_status is null  $whereUser    $whereTag";     
        }
...
foreach($lab_part as $tpart) { $tpart->part_id =$tpart->tlp_id;  ?>
 ...
 // Closure Code Details
    ...
    public function save_closure_code_observation(Request $request)
    {
     ...
     $Tag = DB::update("update tagging_master set closure_codes='$closure_codes',closure_date ='$closure_date',case_close='1' where TagId ='$TagId'");
...
WHERE 1=1 "; 
    ...
    public function vehicle_save_observation(Request $request)
    {
        ...
        system_sw_version='$system_sw_version', Product='$product_name', Model='$model_name', product_id='$Product', model_id='$Model' where TagId ='$RTagId'");
    ...
    public function complaint_save_observation(Request $request)
    {
        ...
        {   
            ...
            final_job_status='$final_job_status' where TagId ='$RTagId'");
    ...
    public function ho_transfer_job(Request $request)
    {
        ...
        if(empty($whereTag))
        {
           ...
           $tag_data_qr = "select tm.*,dm.dist_name,tsc.center_name from $from_table tagging_master tm $on_table left join tbl_service_centre tsc on tm.center_id = tsc.center_id left join district_master dm on tm.dist_id = dm.dist_id where special_approval_status='0'";
        }
        else
        {
