...
class InwardInventoryController extends Controller
{
    ...
    public function cancel_inw_inv(Request $request)
    {
        ...
        $created_at     =   date('Y-m-d H:i:s');
        DB::beginTransaction();
        $inw_det = InwardInventory::whereRaw("inw_id='$inw_id'")->first();
        $po_det = RequestInventory::whereRaw("inward_id='$inw_id'")->first();
        $request_arr = InwardInventoryPart::whereRaw("inw_id='$inw_id'")->get();
        foreach($request_arr as $part)
                    {
                        $spare_id = $part->spare_id;
                        $stock_qty = $part->item_qty;
        ...
        RequestInventory::whereRaw("inward_id='$inw_id'")->update(array('inward_status'=>'0','inward_date'=>$created_at,'inward_by'=>$created_by,'inward_id'=>null));    
        
        
        DB::commit();
        Session::flash('message', "Stock Inward {$inw_det->inw_no} for PO No. {$po_det->req_no} Deleted Successfully.");
        Session::flash('alert-class', 'alert-success');
