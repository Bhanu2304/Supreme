...
class OutwardInventoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if(empty($whereTag))
        {
            $po_job_qry =   "SELECT tm.*,tm.created_at,tsp.part_po_no,tsp.po_type,tsp.part_name,tsp.part_no,tsp.color,tsp.hsn_code,tsp.gst,tsp.customer_price,tsp.spare_id,bm.brand_name,cat.category_name,pm.product_name,mm.model_name FROM tagging_master tm INNER JOIN  tagging_spare_part tsp ON tm.tagid = tsp.tag_id 
        ...
        if(empty($whereTag1))
        {
            ...
            $po_sc_arr =   DB::select("SELECT *,sri.created_at FROM `sc_request_inventory`  sri
            ...
            WHERE part_status_pending ='1' and date(sri.created_at)= curdate()");
            ...
            inner join product_master pm on sri.product_id = pm.product_id
            inner join model_master mm on sri.model_id = mm.model_id
            WHERE part_status_pending ='1' $whereTag1");
        }
    ...
    public function approve_part_po_sc(Request $request)
    {
        ...
        if($ApproveRequestInventory->save())
        {
            ...
            if($RequestInventoryPart->insert($request_arr))
            {
                ...
                {
                    ApproveRequestInventoryPart::whereRaw("req_id='$req_id'")->update(array('approve_id'=>$approve_id,'challan_no'=>$new_challan_no));
