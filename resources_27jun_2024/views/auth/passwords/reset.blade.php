...
<head>
   ...
   <meta name="description" content="Supreme Template">
    ...
    <div class="tab-content">
        @if (session('status'))
            ...
            <form method="POST" action="{{ route('password.request') }}" aria-label="{{ __('Register') }}" onsubmit="return validate_register_credential_first()" >
                ...
                <input type="email" id="email" name="email" required placeholder="mail@domain.com" value="{{ $email}}" class="form-control middle" readonly="">
                @if ($errors->has('email'))
                ...
                <input type="password" id="password_confirm" name="password_confirmation"  placeholder="re-password" class="form-control bottom">
