...
order BY field_name,sub_field_name";
    ...
    public function save_tagging(Request $request)
    {
        ...
        $place_det = PincodeMaster::whereRaw("pin_id='$place'")->first();
        $place = $place_det->place;
        $place_id = $place_det->place_id;
        ...
        $taggingArr->sr_no=$sr_no;
            ...
            if($taggingArr->save()){
                ...
                //$warranty_card_copy = addslashes($request->input('warranty_card_copy'));
                    if(!empty($_FILES['warrenty_card_copy']['name']))
                    {

                        $ext= $today_date.substr($_FILES['warrenty_card_copy']['name'],strrpos($_FILES['warrenty_card_copy']['name'],'.'),strlen($_FILES['warrenty_card_copy']['name'])); ;
                        Storage::disk('supreme')->put("$TagId/warrenty_card_copy"."$ext", file_get_contents($_FILES['warrenty_card_copy']['tmp_name']));
                        $taggingArr = array();
                        $taggingArr['warranty_card_copy']="warrenty_card_copy$ext";
            ...
            if($entry_type=='calling')
            {
                return redirect('tagging-master');
            }
            else
            {
                return redirect('walking-master');
            }
      //  }
