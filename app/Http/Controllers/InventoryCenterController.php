...
class InventoryCenterController extends Controller
{
    ...
    {
        Session::put("page-title","Center Inventory");
    ...
    {   
        ...
        $spare_part_det = SparePart::whereRaw("brand_id ='$brand_id' and product_category_id='$product_category_id' and product_id='$product_id' and model_id='$model_id' and spare_id='$spare_id'")->first();
        $part_no = $spare_part_det->part_no;
        $spareArr            =   new InventoryCenterAdd();
        $spareArr->spare_id=$spare_id;
        ...
        {
            $spare_det_exist = InventoryCenter::whereRaw("center_id='$center_id' and brand_id='$brand_id' and product_category_id='$product_category_id' and product_id='$product_id' and model_id='$model_id' and part_name='$part_name' and part_no='$part_no' ")->first();
            if($spare_det_exist)
            {
                $main_inv = array();
                ...
                {
                    Session::flash('message', "Part $part_no Added to Inventory Successfully.");
    ...
    {
        Session::put("page-title","Center Inventory Details");
        $inv_id = base64_decode($request->input('inv_id'));
        $spare_det_exist = InventoryCenter::selectRaw("brand_id,product_category_id,product_id,model_id,spare_id,part_name,part_no,hsn_code")->whereRaw("inv_id='$inv_id'")->first();
...
WHERE  part_name='$part_name' AND part_no='$part_no' 
ORDER BY ti.created_at DESC";
        //echo $qr;exit;
        $inv_arr           =   DB::select($qr);
        
        $qr_inw = "SELECT ti.*,tsc.center_name,DATE_FORMAT(iisc.created_at,'%d-%b-%Y %H:%i') created_at,bm.brand_name,cm.category_name,
pm.product_name,mm.model_name FROM `inward_inventory_sc` iisc
INNER JOIN `inward_inventory_particulars_sc` ti  ON iisc.inwsc_id = ti.inwsc_id
INNER JOIN tbl_service_centre tsc ON iisc.center_id = tsc.center_id AND sc_status='1' 
INNER JOIN brand_master bm ON ti.brand_id = bm.brand_id and bm.brand_id='$brand_id'  
inner join product_category_master cm on ti.product_category_id= cm.product_category_id and cm.product_category_id='$product_category_id'  
INNER JOIN product_master pm ON ti.product_id = pm.product_id  and pm.product_id='$product_id' 
INNER JOIN model_master mm ON ti.model_id = mm.model_id and mm.model_id='$model_id' 

WHERE  part_name='$part_name' AND part_no='$part_no' 
ORDER BY ti.created_at DESC";
        //echo $qr;exit;
        $inw_arr           =   DB::select($qr_inw);
...
ON tic.center_id = tsc.center_id WHERE stock_status='1'");
    ...
    public function search_center_inventory(Request $request)
    {
        ...
        {
            $whereRaw = " and bm.brand_id='$brand_search'";
        }
        if(!empty($product_category_search))
        {
            $whereRaw .= " and cm.product_category_id='$product_category_search'";
        }
        if(!empty($product_search))
        {
            $whereRaw .= " and pm.product_id='$product_search'";
        }
