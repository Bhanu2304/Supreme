...
class InwardInventoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        {  
            ...
            $whereTag2 .= " and date(inw.inw_date) between '$from_date3' and '$to_date3'";
        }
        else
        {
            $whereTag2 .= " and date(inw.inw_date) =curdate() ";
        }
           
            $req_arr3           =   DB::select("SELECT * FROM `inward_inventory` inw INNER JOIN inward_inventory_particulars iip ON inw.inw_id = iip.inw_id
            ...
            INNER JOIN tbl_spare_parts tsp ON iip.spare_id = tsp.spare_id where 1=1  $whereTag2 ");

            $search_qry ="SELECT * FROM `request_inventory` inw 
            ...
            LEFT JOIN supplier_master sm ON inw.supplier_id = sm.id
                        where 1=1 and  part_reject is null $whereTag ";
            ...
            foreach($req_arr2 as $req)
            {
                ...
                $sel_part = "SELECT * FROM `request_inventory_particulars` rip where req_id='{$req->req_id}' $wherepart  limit 1";
                $req_first_part           =   DB::select($sel_part); 
                //print_r($req_first_part[0]);exit;
                $spare_id = $req_first_part[0]->spare_id;
                #echo "brand_id='$brand_id' and model_id='$model_id' and spare_id='$spare_id'";echo "<br>";
    ...
    public function po_index(Request $request)
    {
        ...
        #echo $req_id;die;
        $po_inv = RequestInventory::whereRaw("req_id='$req_id'")->first();
        $po_inv_parts = RequestInventoryPart::whereRaw("req_id='$req_id'")->get();
        ...
        foreach($po_inv_parts as $pinv)
        {
            ...
            $part_list = SparePart::whereRaw("model_id='$model_id'")->get();
        ...
        $url = $_SERVER['APP_URL'].'/po-inw-inv-entry';
        return view('po-inw-inv-entry')
        ->with('supplier_master', $supplier_master)
        ->with('brand_arr', $brand_arr)
        ->with('part_arr', $part_arr)
        ->with('po_inv', $po_inv)
    ...
    public function save_req_inv(Request $request)
    {
        ...
        $po_req = RequestInventory::whereRaw("req_id='$req_id'")->first();
        $po_inv_parts = RequestInventoryPart::whereRaw("req_id='$req_id'")->get();
        $supplier_id = $request->input('supplier_name');
        ...
        foreach($po_inv_parts as $po_inv)
        {
            ...
            echo $hsn_code_to_be_updated = $part_list['hsn_code'.$req_part_id]; exit;
            if(!empty($hsn_code_to_be_updated))
            {
                $SparePart = SparePart::whereRaw("spare_id='$spare_id'")->update(array('hsn_code'=>$hsn_code_to_be_updated));
            }
        }
        ...
        $inward_qty = $po_order[0]['item_qty'];
        #print_r($po_order);die;
        
            $InwardInventory = new InwardInventory();
           ...
           // $RequestInventory->center_id= $Center_Id;
            ...
            //echo $new_request_no; exit;
                $InwardInventory->inw_no= $new_request_no;
                $InwardInventory->inw_date= date('Y-m-d');
            ...
            if($InwardInventory->save())
            {
                ...
                if($RequestInventoryPart->insert($request_arr))
                {
                    ...
                    $req = RequestInventory::whereRaw("req_id='$req_id'")->update(
                            array(
    ...
    public function cancel_inw_inv(Request $request)
    {
        ...
        $created_at     =   date('Y-m-d H:i:s');
        DB::beginTransaction();
        #$inw_det = InwardInventory::whereRaw("req_id='$inw_id'")->first();
        #print_r($inw_det);die;
        $po_det = RequestInventory::whereRaw("req_id='$inw_id'")->first();
        
        #$request_arr = InwardInventoryPart::whereRaw("req_id='$inw_id'")->get();
        ...
        #InwardInventory::whereRaw("inw_id='$inw_id'")->delete();
        #InwardInventoryPart::whereRaw("inw_id='$inw_id'")->delete();
        #InventoryItemList::whereRaw("part_inw_id='$inw_id'")->delete();
        RequestInventory::whereRaw("req_id='$inw_id'")->update(array('part_status_pending'=>'0','inward_date'=>$created_at,'inward_by'=>$created_by,'part_reject'=>'1'));    
        
        
        DB::commit();
        Session::flash('message', "Stock Inward {$inw_det->inw_no} for PO No. {$po_det->req_no} Cancel Successfully.");
        Session::flash('alert-class', 'alert-success');
