    ...
    */
    ...
    public function index(Request $request)
    {
        ...
        if(empty($whereTag))
        {
            ...
            left join tagging_damage_part tdp ON tm.tagid = tdp.tag_id 
        ...
        $model_qry = "SELECT pm.product_id,pm.product_name FROM product_master pm 
    ...
    public function apply_jobs(Request $request)
    {
        ...
        Session::put("page-title","Apply Jobs");
        $UserType = Session::get('UserType');
        $UserId = Session::get('UserId');
        ...
        if(empty($whereTag))
        {
            ...
            #$req_arr  =   DB::select("SELECT * FROM inward_inventory ii LEFT JOIN inward_inventory_particulars iip ON ii.inw_id = iip.inw_id");
            $qry = "SELECT tm.*,dm.dist_name,tsc.center_name FROM tagging_master tm 
            -- inner JOIN tagging_damage_part tdp ON tm.tagid = tdp.tag_id 
            LEFT JOIN tbl_service_centre tsc ON tm.center_id = tsc.center_id 
            LEFT JOIN district_master dm ON tm.dist_id = dm.dist_id WHERE job_reject='0' and job_accept='1' and claim_status is null AND closure_codes IS NOT NULL order  by created_at desc";
            #echo $query;die;
            ...
            -- inner JOIN tagging_damage_part tdp ON tm.tagid = tdp.tag_id 
            LEFT JOIN tbl_service_centre tsc ON tm.center_id = tsc.center_id 
            LEFT JOIN district_master dm ON tm.dist_id = dm.dist_id WHERE job_reject='0' and job_accept='1' and claim_status is null AND closure_codes IS NOT NULL $whereTag";
            $req_arr  =   DB::select($qry);
            #echo $query;die;
            #$req_arr  =   DB::select("SELECT * FROM inward_inventory ii LEFT JOIN inward_inventory_particulars iip ON ii.inw_id = iip.inw_id where 1=1 $whereTag");
        }
        ...
        $qr2 = "SELECT tsc.center_id,center_name,asc_code FROM tbl_service_centre  tsc
            INNER JOIN users us ON tsc.email_id = us.email WHERE sc_status='1' $center_qr order by center_name"; 
        $asc_master           =   DB::select($qr2);
        ...
        $url = $_SERVER['APP_URL'].'/apply-jobs';
    ...
    function save_claim_jobs(Request $request)
    {
        $selected = $request->input('apply');

        if (empty($selected)) {
            return response()->json(['message' => 'No checkboxes selected.'], 400);
        }
        ...
        foreach ($selected as $tagId) {
            ...
            DB::table('tagging_master')
                ->where('TagId', $tagId)
                ->update(['claim_status' => '1','claim_gen_date'=>$created_at]);
        }
    ...
    public function view_apply(Request $request)
    {
        ...
        Session::put("page-title","View Apply Jobs");
        $UserType = Session::get('UserType');
        $UserId = Session::get('UserId');
        ...
        {   
            $from_date_arr = explode('-',$from_date);  krsort($from_date_arr); $from_date1 = implode('-',$from_date_arr);
            $to_date_arr = explode('-',$to_date);  krsort($to_date_arr); $to_date1 = implode('-',$to_date_arr);
            $whereTag .= " and DATE(tm.claim_gen_date) between '$from_date1' and '$to_date1'";
        }
        ...
        if(empty($whereTag))
        {
            ...
            -- inner JOIN tagging_damage_part tdp ON tm.tagid = tdp.tag_id 
            LEFT JOIN tbl_service_centre tsc ON tm.center_id = tsc.center_id 
            LEFT JOIN district_master dm ON tm.dist_id = dm.dist_id WHERE job_reject='0' and job_accept='1' and claim_status='1' AND date(claim_gen_date)= curdate()";
            #echo $query;die;
            $req_arr  =   DB::select($qry);
        ...
        $url = $_SERVER['APP_URL'].'/view-apply';
    ...
    public function account_statement(Request $request)
    {
        ...
        Session::put("page-title","Account Statement");
        $UserType = Session::get('UserType');
        $UserId = Session::get('UserId');
        ...
        {   
            $from_date_arr = explode('-',$from_date);  krsort($from_date_arr); $from_date1 = implode('-',$from_date_arr);
            $to_date_arr = explode('-',$to_date);  krsort($to_date_arr); $to_date1 = implode('-',$to_date_arr);
            $whereTag .= " and DATE(td.disperse_date) between '$from_date1' and '$to_date1'";
        }
        ...
        if(empty($whereTag))
        {
            ...
            $qry = "SELECT * FROM `tbl_disperse` td INNER JOIN tagging_master tm ON td.tag_id = tm.tagid where date(td.disperse_date)= curdate()";
            $req_arr  =   DB::select($qry);
            
        }else{

            $qry = "SELECT * FROM `tbl_disperse` td INNER JOIN tagging_master tm ON td.tag_id = tm.tagid where 1=1 $whereTag";
            $req_arr  =   DB::select($qry);
        ...
        $url = $_SERVER['APP_URL'].'/account-statement';
