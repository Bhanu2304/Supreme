...
class ManagerController extends Controller
{
    ...
    {
        Session::put("page-title","Add ASC Manager");
        ...
        if($UserType!='Admin')
        {
            ...
            $center_qr1 = " where  ce.center_id ='$Center_Id'"; 
        }

        $qr = "SELECT man_id,email,pass,phone,man_name,man_status,DATE_FORMAT(se.created_at,'%d-%b-%Y') created_at,center_name
        FROM  `tbl_manager` se left join tbl_service_centre ce on se.center_id = ce.center_id $center_qr1";
    ...
    public function save_man(Request $request)
    {
        ...
        if(!empty($data_em))
        {
            Session::flash('error', "Email Already Associated With Other Account.");
            Session::flash('alert-class', 'alert-danger');
            return back();
        }
        else if(!empty($data_mo))
        {
            Session::flash('error', "Mobile Already Associated With Other Account");
            Session::flash('alert-class', 'alert-danger');
            return back();
        }
        else if(empty($center_id))
        {
            Session::flash('error', "Please Select Service Center First");
            Session::flash('alert-class', 'alert-danger');
            return back();
        }
        else
        {
            ...
            if($userArr->save()){
                ...
                Session::flash('message', "ASC Manager Created Successfully.");
                Session::flash('alert-class', 'alert-success');
            }
            else{
                Session::flash('error', "ASC Manager Creation Failed. Please Try Again");
                Session::flash('alert-class', 'alert-danger');
                return back();
            } 
    ...
    {
        Session::put("page-title","Edit ASC Manager");
        $UserType = Session::get('UserType');
        $UserId = Session::get('UserId');
        $Center_Id = Auth::user()->table_id;
        ...
        if($UserType!='Admin')
        {
            $center_qr = " and tsc.center_id ='$Center_Id'"; 
        }
    ...
    public function update_man(Request $request)
    {
        ...
        else
        {
            ...
            if(User::whereRaw("email='{$data['email']}'")->update($userArr)){
                ...
                Session::flash('message', "ASC Manager Details Updated Successfully.");
                Session::flash('alert-class', 'alert-success');
            }
            else{
                Session::flash('error', "ASC Manager Details Update Failed. Please Try Again");
                Session::flash('alert-class', 'alert-danger');
            } 
...
inner JOIN `region_master` regm ON ar_map.region_id=regm.region_id $region_str";
    ...
    public function save_map_area(Request $request)
    {
        ...
        $dist_ids = $request->input('dist_id'); 
        $region_id = $request->input('region_id'); 
        ...
        if($user_type=='RSM' || $user_type=='NSM')
        {
            ...
            else
            {
                ...
                $data_state_exist = RegAreaMaster::whereRaw("reg_man_id='$reg_man_id' and region_id='$region_id' and ")->first();
        ...
        else if($user_type=='ASM' ||$user_type=='Coordinator' ||$user_type=='Store' ||$user_type=='BSM' ||$user_type=='Account' ||$user_type=='Reception' )
        {
            ...
            {   
                if($state_id == "All")
                {
                    ...
                    $all_area =DistrictMaster::whereRaw("1=1 $state_str")->get();
                    $RegionalManagerMaster = RegionalManagerMaster::whereRaw("reg_man_id='$reg_man_id'")->first();
                ...
                {   
                    ...
                    else if($dist_id=='All')
                    {
                        ...
                        if($state_id!='All')
                        {
                            $state_str = " and state_id= '$state_id'";
                        }  
                        $all_area =DistrictMaster::whereRaw("1=1 $state_str")->get();
                        $RegionalManagerMaster = RegionalManagerMaster::whereRaw("reg_man_id='$reg_man_id'")->first();
    ...
    public function get_map_area(Request $request)
    {
        ...
        if(!empty($state_id) && $state_id!='All')
        {
