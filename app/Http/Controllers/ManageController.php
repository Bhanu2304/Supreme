...
class ManageController extends Controller
{
    ...
    public function index()
    {
        ...
        if($UserType=='Admin')
        {
            $where_user_type_in = " and UserType in ('Admin','ServiceCenter','Supreme Coordinator','Manager','ASM','RSM','NSM','Account','Store')";
        }
        else if($UserType=='ServiceCenter')
        {
            $Center_Id = Auth::user()->table_id;
            $where_user_type_in = " and UserType in ('ServiceEngineer','Store') and table_id='$Center_Id'";
        }
 ...
 ORDER BY page_name");
    ...
    public function closure_code(Request $request)
    {
        ...
        #if ($request->has('closure_code'))
        {
            ...
            $create = ClosureCode::create(['closure_code' => $closure_code,'amount'=>$amount,'description'=>$description,'created_by' => $UserId]);
            if($create)
            {
    ...
    public function edit_closure(Request $request)
    {
        ...
        if($request->isMethod('post')) 
        {
            ...
            $update = ClosureCode::where('id', $se_id)->update(['closure_code' => $closure_code,'amount'=>$amount,'description'=>$description,'status'=>$status,'updated_by'=>$UserId]);
            if($update)
            {
    ...
    {
        Session::put("page-title","Supplier Management");
        $UserType = Session::get('UserType');
        $UserId = Session::get('UserId');
        ...
        #if ($request->has('closure_code'))
        {
            ...
            $data_json =   StateMaster::whereRaw("state_id='$state'")->first(); 
            $data_sc = json_decode($data_json);
            $state_code = $data_sc->state_code;


            #$find_max_supplier_no=SupplierMaster::selectRaw('supplier_code')->first();
            #$find_max_supplier_no = SupplierMaster::max('supplier_code');
            $find_max_supplier_no = SupplierMaster::count('supplier_code');
            #echo $find_max_supplier_no;die;
            $supplier_no_date = date('y');
            $new_supplier_no = "";
            if($find_max_supplier_no == 0)
            {   
                $new_supplier_no = "SUP$supplier_no_date".'0001';
            ...
            else
            {
                #echo "not empty";die;
                $str_no = "0000";
                $no = $find_max_supplier_no;
                $no = $no+1;
                $len = strlen($str_no);
                $newlen = strlen("$no");
                $new_no = substr_replace($str_no, $no, $len-$newlen,$newlen);

                $new_supplier_no = "SUP$supplier_no_date".$new_no;
            ...
            #echo $new_supplier_no;die;
            
            
            $create = SupplierMaster::create(['supplier_code'=>$new_supplier_no,'supplier_name' => $supplier_name,'address'=>$address,'state'=>$state,'state_code'=>$state_code,'phone_no'=>$phone_no,'email'=>$email,'gst_code'=>$gst_no,'created_by' => $UserId,'created_at' => date('Y-m-d H:i:s')]);
            if($create)
            {
                Session::flash('message', "Supplier Created Successfully.");
                Session::flash('alert-class', 'alert-success');
            }else{
                Session::flash('error', "Supplier Creation Failed. Please Try Again");
                Session::flash('alert-class', 'alert-danger');
            }
        ...
        $qr = "SELECT sum.*,sm.state_name from supplier_master sum left JOIN `state_master` sm ON sm.state_id= sum.state";
    ...
    public function edit_supplier(Request $request)
    {
        ...
        if($request->isMethod('post')) 
        {
            ...
            $update = SupplierMaster::where('id', $se_id)->update(['supplier_name' => $supplier_name,'address'=>$address,'state'=>$state,'state_code'=>$state_code,'phone_no'=>$phone_no,'email'=>$email,'gst_code'=>$gst_no,'active_status'=>$status,'updated_by'=>$UserId,'updated_at' => date('Y-m-d H:i:s')]);
            if($update)
            {
                ...
                Session::flash('error', "Supplier Update Failed. Please Try Again");
                Session::flash('alert-class', 'alert-danger');
            }
        ...
        $supplier_id = base64_decode($request->input('supplier_id')); 
        $data_json = SupplierMaster::whereRaw("id='$supplier_id'")->first();
        $data = json_decode($data_json,true);

        $state_json           =   StateMaster::whereRaw("state_status=1")->orderByRaw('state_name ASC')->get(); 
        $state_master = json_decode($state_json,true);
        
        $url = $_SERVER['APP_URL'].'/edit-supplier';
        //print_r($data); exit;
        return view('edit-supplier')->with('data',$data)
        ->with('state_master',$state_master)
        ->with('url', $url);
