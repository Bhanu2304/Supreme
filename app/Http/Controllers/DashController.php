...
class DashController extends Controller
{
    ...
    public function brand_dashboard(Request $request)
    {
        ...
        {
            $whereUser .= " and tm.center_id ='$Center_Id'";
            $whereCenter .= " and tsc.center_id ='$Center_Id'";
            $filter_tag .= "and center_id ='$Center_Id'";
        }
        else if($UserType=='ServiceEngineer')
        {
            $whereUser = "and tm.se_id='$UserId' and tm.center_id='$Center_Id'";
        ...
        $qr2 = "SELECT tsc.center_id,center_name,asc_code FROM tbl_service_centre  tsc
            INNER JOIN users us ON tsc.email_id = us.email
            WHERE sc_status='1' $whereCenter  order by center_name"; 
        $asc_master           =   DB::select($qr2); 
        ...
        if(!empty($whereTag))
        {
            ...
            $tag_data_qr = "select tm.*,center_name,state_name from $from_table tagging_master tm $on_table inner join tbl_service_centre sc on tm.center_id = sc.center_id INNER JOIN state_master sm ON sm.state_id = sc.state where 1=1 $whereTag $whereUser   ";
            $DataArr = DB::select("$tag_data_qr");
