...
class RequestInventoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        {   
            $whereTag .= " and ri.supplier_id = '$supplier_name' ";
        }
        ...
        if(empty($whereTag))
        {
            ...
            $req_arr           =   DB::select("SELECT *,brand_name,category_name,product_name,model_name,ri.remarks,tsp.part_name,tsp.part_no,rip.color,date_format(ri.req_date,'%d-%m-%Y') created_at,date_format(ri.approve_date,'%d-%m-%Y') approve_date,sm.supplier_name,ri.qty FROM request_inventory ri 
            ...
            $qry_find_po = "SELECT *,brand_name,category_name,product_name,model_name,ri.remarks,tsp.part_name,tsp.part_no,rip.color,date_format(ri.req_date,'%d-%m-%Y') created_at,date_format(ri.approve_date,'%d-%m-%Y') approve_date,sm.supplier_name,ri.qty FROM request_inventory ri 
            INNER JOIN request_inventory_particulars rip ON ri.req_id = rip.req_id
            inner join brand_master bm on ri.brand_id = bm.brand_id
            inner join product_category_master cat on ri.product_category_id = cat.product_category_id
            inner join product_master pm on ri.product_id = pm.product_id
        ...
        ->with('supplier_id', $supplier_name)
    ...
    {  
        ...
        $po_inv_parts = RequestInventoryPart::whereRaw("brand_id='$brand_id'  and spare_id='$random_no'")->latest()->first();
        ...
        #print_r($consumption_arr);die;
        #echo "SELECT sum(issued_qty) cnsptn FROM outward_inventory where brand_id='$brand_id'  and spare_id='$random_no'  ";die;
        ...
        <div class="form-row" id="part_div<?php echo $random_no;?>">
            ...
            <div class="col-md-1">
                <div class="position-relative form-group"><label for="examplePassword11" class="">Total Qty. <font color="red">*</font></label>
                    <input type="text" autocomplete="off" name="SparePart[<?php echo $random_no;?>][total_qty]" id="total_qty<?php echo $random_no;?>"  onkeypress="return checkNumber(this.value,event)" class="form-control" required="" />
                </div>
            </div>
