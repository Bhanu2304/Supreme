...
order BY field_name,sub_field_name";
	...
	public function downloadChallan(Request $request)
    {
		...
		if($challan_no != "")
		{
			...
			$part_arr[] = array('part_name'=> $part_name,'hsn_code'=>$hsn_code,'qty'=>$qty,'total'=>$amount);
	...
	public function generateChallansc(Request $request)
    {
        $ApproveId = $request->input('approve_id');
		...
		if($invoice_id != "")
		{
			$record = OutwardPending::whereRaw("invoice_id='$invoice_id'")->first();
			$center_id = $record->center_id; 
			$part_arr = OutwardPending::whereRaw("invoice_id='$invoice_id'")->get();
	...
	public function downloadChallansc(Request $request)
    {
        $ApproveId = $request->input('approve_id');
...
public function invoice_mpart_pdf(Request $request)
	...
	public function po_invoice_pdf(Request $request)
    {
        ...
        foreach($record_arr_det as $record)
        {
			...
			#print_r($supplier_detail);die;
			

			$record_arr[] = $record;
        }

		$po_no = $record_arr_det[0]->po_no;
		#echo "SELECT * FROM sc_request_inventory WHERE req_no='$po_no' limit 1";die;
		$out_inventory           =   DB::select("SELECT * FROM sc_request_inventory WHERE req_no='$po_no' limit 1");
		$supplier_id = $out_inventory[0]->supplier_id;
		...
		#print_r($data);die;
			...
			if($type == "preview")
			{
				...
				$html = view('po-pdf', compact('record_arr', 'sc_details', 'data', 'image_base64','supplier_detail'))->render();
				$pdf = PDF::loadHTML($html);
	...
	public function po_invoice_pdf_new(Request $request)
    {
        ...
        foreach($record_arr_det as $record)
        {
			...
			$user_det = User::whereRaw("id='$center_id'")->first();
	...
	public function po_invoice_pdf_sc_new(Request $request)
    {
        $req_id = base64_decode($request->input('req_id'));
		$type = $request->input('type');
        ...
        #$record_arr_det = OutwardInvoice::whereRaw("req_id='$req_id'")->get();
		//echo "SELECT * FROM request_inventory_particulars WHERE req_id='$req_id' ";die;
		$record_arr_det           =   DB::select("SELECT * FROM sc_request_inventory_particulars WHERE req_id='$req_id' "); #
		#print_R($record_arr_det);die;
        ...
        foreach($record_arr_det as $record)
        {
			...
			$record_arr_in    =   DB::select("SELECT * FROM sc_request_inventory WHERE req_id='$req_id' ");
			$remarks = $record_arr_in[0]->remarks;
			$record->inv_remarks = $remarks;
