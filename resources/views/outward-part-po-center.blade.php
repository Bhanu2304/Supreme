...
function updateTableRows(value, index,maxQty) 
{
    ...
    if (qty > maxQty) {
        //qty = maxQty;
        document.getElementById(`issued_qty${index}`).value = maxQty; // Update the input field to the max valueissued_qty439
             ...
             <div class="main-card mb-3 card">
                    ...
                    <div class="card-body">                                        
                        ...
                        <table border="1" style="font-size:13px;" id="table_id">
                            ...
                            <tbody>
                            <?php   $srno = 1; #print_r($po_part_master);
                                    foreach($po_part_master as $po_job)
                                    {
                                        echo '<tr id="tr'.$po_job->req_part_id.'">';
                                            echo '<td>'.$srno++.'</td>';
                                            echo '<td>'.date('d-M-y',strtotime($po_job->req_date)).'</td>';
                                            ...
                                            echo '<td>';
                                            echo '<input type="text" style="width:50px;" name="asc_amount" id="asc_amount'.$po_job->req_part_id.'"  onkeypress="return checkNumber(this.value,event)" value="'.$po_job->asc_amount.'">';
                                            echo '</td>';
                                            echo '<td>';
                                            echo '<input type="text" style="width:50px;" name="customer_amount" id="customer_amount'.$po_job->req_part_id.'"  onkeypress="return checkNumber(this.value,event)" value="'.$po_job->customer_amount.'">';
                                            echo '</td>';
                                            echo '<td>';
                                            ...
                                            echo '</td>';
                                            echo '<td id="pen_parts'.$po_job->req_part_id.'">';
                                            echo $po_job->qty_pending;
                                            echo '</td>';
                                            echo '<td>';?>
                                            <input type="text" style="width:50px;" onkeypress="return checkNumber(this.value,event)" autocomplete="off" id="issued_qty<?php echo $po_job->req_part_id; ?>" oninput="updateTableRows(this.value, <?php echo $po_job->req_part_id; ?>,<?php echo $po_job->qty_pending;?>)" name="issued_qty<?php echo $po_job->req_part_id; ?>"  />
                                            <?php echo '</td>';
                                            echo '<td>';?>
                                            <input style="width:50px;" onkeypress="return checkNumber(this.value,event)" autocomplete="off" type="text" id="discount<?php echo $po_job->req_part_id; ?>" name="discount<?php echo $po_job->req_part_id; ?>"  />
                                            <?php echo '</td>';
                                            echo '<td>';?>
                                            <input autocomplete="off" type="text" id="remarks<?php echo $po_job->req_part_id; ?>" name="remarks<?php echo $po_job->req_part_id; ?>"  />
                                            <?php echo '</td>';
                                            echo '<td>';
                                            if(!empty($po_job->bal_qty))
                                            {
                                                echo '<a href="#" onclick="approve('."'{$po_job->req_part_id}'".')">Issue</a>';
                                            }
                                            else
                                            {
                                            ...
                                            echo '</td>';
                                            #echo '<td><a href="#" onclick="cancel('."'{$po_job->req_part_id}'".')">Cancel</a></td>';
                                        echo '</tr>';
                                    }
                            ?>
                            </tbody>
                        ...
                        <table border="1">
                            ...
                            <tbody>
                                ...
                                { ?>
                                <tr  id="sr<?php echo $po_inv_part->req_part_id; ?>" class="table-row-<?php echo $a; ?>">
                                <?php
                                    #print_r($po_inv_part);die;
                                    
                                    $item_qty = $po_inv_part->qty_pending;
                                    ...
                                    ?>
                                        ...
                                        <!-- <tr  id="row<?php //echo $b; ?>" id="item_list_<?php //echo $b; ?>"> -->
                                            ...
                                            <td>
                                                <?php echo $po_inv_part->model; ?>
                                            </td>
                                            <td>
                                                <?php echo $po_inv_part->part_no; ?>
                                            </td>
                                            <td>
                                                <?php foreach($po_inv_part->srno_list as $srno_det) { ?>
                                                <input type="checkbox" name='srno_list[<?php echo $srno_det->id;?>]' value="<?php echo $srno_det->id; ?>" placeholder="Sr No."/><?php echo $srno_det->srno; ?>
                                                <?php } ?>
                                            </td>
                                        </tr>
                                    <?php  
...
<script>
    ...
    function approve(part_id)
    {
        ...
        var customer_amount = $('#customer_amount'+part_id).html();
        var asc_amount = $('#asc_amount'+part_id).html();
        ...
        if(discount!=='')
        {
            var discount_new = parseFloat(discount);
            if(discount_new>100)
            {
        ...
        data['srno_list[]'].push($(this).val());
        });
        
        data['issued_qty'] = issued_qty;
        data['part_id'] = part_id;
        data['remarks'] = remarks;
        data['discount'] = discount;
        data['customer_amount'] = customer_amount;
        data['asc_amount'] = asc_amount;
    ...
    jQuery.ajax({
              ...
              success: function(result){
                  if(result==='1')
                  {
                      $('#tr'+part_id).remove();
                      $('#sr'+part_id).remove();
                      $('#succ').html('Spare Part Approved Successfully.');
                      $('#succ').show();
                      $('#error').hide();
                  }
                  else if(result==='2')
                  ...
                  {
                      $('#succ').html('Sr. No. Selected More Than Part Issued.');
                      $('#succ').show();
                      $('#error').hide();
                  }
                  else
                  {
                      $('#succ').hide();
                      $('#error').html('Spare Part Not Approved.');
                      $('#error').show();
                  }
