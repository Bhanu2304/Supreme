...
function job_accept(tagId)
{
    $('#scc').html("");
    $('#err').html("");
    $.post('approved-special-job',{tagId:tagId}, function(resp){
        //const obj = JSON.parse(resp);

        if(resp==='1')
        {
            $('#scc').html('<h5><font color="green">Job Approved Successfully.</font></h5>');
            //$('#td'+tagId).html('<a onclick="job_reject('+"'"+tagId+"'"+');" href="#">Reject</a>');
            $('#td'+tagId).html("Approved");
            $('#tr'+tagId).remove();
        }
        else
        { 
            $('#err').html('<h5><font color="red"> Job Case Already Approved</font></h5>');
        }
            ...
            <div class="tab-content">
                <div class="tab-pane tabs-animation fade show active" id="tab-content-0" role="tabpanel">
                    ...
                    <div class="main-card mb-3 card">
                        <div class="card-body"><h5 class="card-title">ASC Transferred Job Approval</h5>
                            ...
                            <p id="scc">@if(Session::has('message'))<h5><font color="green"> {{ Session::get('message') }}</font></h5> @endif </p>
                                 ...
                                 <form method="get" action="{{route('ho-transfer-job')}}" autocomplete="off" class="form-horizontal"  >
 				...
 				<div class="col-md-2">
                    ...
                    <div class="content" id = "content"></div>
                            ...
                            <h5 class="card-title">ASC Transferred Job Details</h5>
                                 ...
                                 <?php $srno = 1;
                                        foreach($DataArr as $record)
                                        {
                                            ...
                                            echo $srno++.'</td>';
                                                ...
                                                {
                                                    echo '<a href="vendor-observation-cl?TagId='.$record->TagId.'&approvaltype='.$record->special_approval_type.'">'.$record->ticket_no.'</a>';
                                                }else{
                                                    echo '<a href="vendor-observation?TagId='.$record->TagId.'&approvaltype='.$record->special_approval_type.'">'.$record->ticket_no.'</a>';
                                                }
                                                #echo "brand-id=>$record->brand_id";
                                                echo '</td>';
                                                echo '<td id="td'.$record->TagId.'">';
                                                if($record->special_approval_status=='0')
                                                {
                                                echo '<a onclick="job_accept('."'".$record->TagId."'".');" href="#">Approve</a>';
                                                }
                                                else
                                                {
                                                    echo 'Approved';
                                                ...
                                                echo '</td>';
                                                echo '<td>';
                                                #echo '<a onclick="ticket_reject('."'".$record->TagId."'".');" href="#">Reject</a>';
                                                echo '</td>';
