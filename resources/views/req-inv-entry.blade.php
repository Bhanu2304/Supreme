         ...
         <div class="tab-pane tabs-animation fade <?php if($tab!=='1') { ?>show active <?php } ?>" id="tab-content-0" role="tabpanel">
             <div class="main-card mb-3 card"> 
                    ...
                    <div class="card-body"><h5 class="card-title">View Purchase Order</h5>
                        <form method="get" action="{{route('req-inv-entry')}}" class="form-horizontal">
                                    ...
                                    <div class="position-relative form-group">
                                        ...
                                        <select id="supplier_name" name="supplier_name"  class="form-control">
                                            ...
                                            <?php foreach($supplier_arr as $supplier) {?>       
                                                <option value="<?php echo $supplier['id']; ?>" <?php if($supplier_id==$supplier['id']){ echo "selected"; } ?>><?php echo $supplier['supplier_name']; ?></option>     
                                            <?php  }?>
                                        </select>
                            ...
                            <?php   $srno = 1;
                                foreach($req_arr as $req)
                                {
                                    ...
                                    echo '<tr>';
                                        ...
                                        echo '<td>'.$req->inward_date.'</td>';
                                        echo '<td>'.$req->inward_qty.'</td>';
                                        echo '<td>';
                                        ...
                                        echo '<td><a href="view-req-inv-entry?req_id='.base64_encode($req->req_id).'">View</a></td>';
                                        echo '<td>';
                                        if($req->part_reject == 1 || $req->inward_status==1)
                                        {

                                        }else{
                                            echo '<a href="edit-req-inv-entry?req_id='.base64_encode($req->req_id).'">Edit</a>';
                                        }
                                        
                                        echo '</td>';
                                        echo '<td>';
                                        echo '<a href="po-invoice-pdf-new?type=preview&req_id='.base64_encode($req->req_id).'">View</a> || ';
                                        echo '<a href="po-invoice-pdf-new?req_id='.base64_encode($req->req_id).'">Download</a>';
                                        echo '</td>';
                                    echo '</tr>';
                                }
                            ?>
                                ...
                                <div class="form-row">
                                    ...
                                    <div class="col-md-2">
                                        <div class="position-relative form-group"><label for="examplePassword11" class="">Part Code <font color="red">*</font></label>
                                            <select multiple="" id="part_code1" name="part_code" class="form-control" onchange="add_part('1',this.value)" >
                                                <option value="">Select</option>
                                            </select>
                                        </div>
                                    </div>
