    ...
    */
    ...
    public function index(Request $request)
    {
        ...
        if($request->isMethod('post'))
        {
            ...
            $product_qr = "SELECT product_id, product_name  FROM product_master WHERE brand_id='4' AND product_status='1' AND LOWER(product_name) = LOWER('$prodct_model') limit 1;";
            ...
            if($UserType=='ServiceCenter')
            {
                ...
                $savedata['center_allocation_date']=date('Y-m-d H:i:s');
                $savedata['center_allocation_by']=$UserId;
                
                $center_det = ServiceCenter::whereRaw("center_id='$center_id'")->first();
                $savedata['asc_code']=$center_det->asc_code;
            ...
            $savedata['ticket_no']=$ticket_no;
            $savedata['job_year']=$year;
            $savedata['job_month']=$month;
            $savedata['sr_no']=$sr_no;
            ...
            if(!empty($error)) {
                ...
                #print_r($DataArr);die;
                Session::flash('message', " $ticket_no Tickets Generate Successfully");
                return redirect("call-registration-form");
            }
