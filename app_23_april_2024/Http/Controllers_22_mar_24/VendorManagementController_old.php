...
<!--<div class="form-row" id="part_div<?php echo $random_no;?>">
    ...
    public function get_tag_parts(Request $request)
    {
        ...
        $tagg_part = TagPart::whereRaw("tag_id='$tag_id' and estmt_approve='0'")->get();
        $lab_part = TaggingSparePart::whereRaw("tag_id='$tag_id' and estmt_approve='0'")->get();
...
foreach($lab_part as $tpart) { $tpart->part_id =$tpart->tlp_id;  ?>
    <tr id="tr<?php echo $tpart->part_id; ?>">
        ...
        <select id="part_name<?php echo $tpart->part_id; ?>"   class="form-control" onchange="get_partno('<?php echo $tpart->part_id; ?>',this.value)" >
            <option value="">Select</option>
            <?php
                    foreach($lab_arr as $part)
                    {
                        ?><option value="<?php echo $part->symptom_name; ?>" <?php if($part->symptom_name==$tpart->symptom_name) { echo 'selected';} ?>><?php echo $part->symptom_name; ?></option>     
            <?php   }
            ?> 
        </select>
        </td>
...
exit;    }
        ...
        foreach($SparePart_arr as $part_det)
        {
           ...
           // $exp = explode("-",$new_spare_id);
            ...
            if($part_type=='part')
            {
                ...
                if(!empty($SparePart) && !empty($newpart_no))
                {   
                    $TagPart  =   array(); 
    ...
    {  
        ...
        if($flag)
        {
            $tagg_part_npc = TagPart::whereRaw("tag_id='$TagId' and npc_req='3'")->get();
            $tagg_lbr_npc = TaggingSparePart::whereRaw("tag_id='$TagId' and npc_req='3'")->get();
...
WHERE 1=1 "; 
 ...
 // End Closure Code Details 
    ...
    public function save_image(Request $request)
    {
        ...
        //DB::insert('insert into image_datas (TagId,file_type,file_name,created_at) values(?,?,?,?)',[$tag_id,$file_type,$file_name,$today_date]);
    ...
    public function destroy_image_data($id){
